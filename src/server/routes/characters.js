...
const { getDb, ensureUniqueCode } = require('../db');
...
// GET /api/characters/by-code/:code - Récupérer par code d'accès
router.get('/by-code/:code', (req, res) => {
    try {
        const db = getDb();
        const char = db.prepare('SELECT id FROM characters WHERE access_code = ?').get(req.params.code.toUpperCase());
...
// GET /api/characters/by-url/:url - Récupérer par URL
