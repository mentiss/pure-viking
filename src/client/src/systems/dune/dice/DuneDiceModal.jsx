...
const DuneDiceModal = ({
    ...
    const handleRoll = useCallback(async () => {
        ...
        try {
            ...
            if (useDetMin && detMode === 'reroll') {
                ...
                const { succes: s1 } = countSuccesses(results,  rang, hasSpec, selectedCompetence.rang);
                const { succes: s2 } = countSuccesses(results2, rang, hasSpec, selectedCompetence.rang);
            ...
            let { succes, complications } = countSuccesses(results, rang, hasSpec, selectedCompetence.rang);
            ...
            const finalResult = {
                results, rang, hasSpec,
                succes: succesTotal, complications, reussite, excedent,
            };

            // Contexte complet pour les effets post-jet
            const postRollContext = {
                ...
                complications,
                // Détermination
                shouldDecrementDet: !!(useDetMin && detMode),
                // Persistance
                historyPayload: {
                    ...
                    roll_definition: {
                        competenceLabel: selectedCompetence
                            ? `${selectedCompetence.key} (rang ${selectedCompetence.rang})`
                            : null,
                        principeLabel: selectedPrincipe
                            ? `${selectedPrincipe.key} (rang ${selectedPrincipe.rang})`
                            : null,
                        difficulte,
                        impulsionsDepensees,
                        menaceGeneree,
                        useDetMin: useDetMin && !!detMode,
            ...
            if (diceConfig.animationEnabled) {
                const animSeq = {
                    mode: 'single',
                    groups: [{ id: 'player-roll', diceType: 'd20', color: 'default', label: `${nbDes}d20`, waves: [{ dice: results }] }],
                    insuranceData: null,
                };
                setPendingResult(postRollContext);
                setAnimSequence(animSeq);
                // Tout le reste se passe dans handleAnimComplete
            } else {
                // Pas d'animation — on exécute tout immédiatement
                applyPostRoll(postRollContext);
            }
        } catch (err) {
    ...
    const applyPostRoll = useCallback((ctx) => {
        const { result, impCout, menaceGeneree, complications, shouldDecrementDet, historyPayload } = ctx;

        // Effets socket
        if (socket && activeGMSession) {
            if (impCout > 0)       socket.emit('update-session-resources', { sessionId: activeGMSession, field: 'impulsions',    delta: -impCout });
            if (menaceGeneree > 0) socket.emit('update-session-resources', { sessionId: activeGMSession, field: 'menace',        delta: menaceGeneree });
            if (complications > 0) socket.emit('update-session-resources', { sessionId: activeGMSession, field: 'complications', delta: complications });
        }

        // Détermination
        if (shouldDecrementDet) {
            onCharacterUpdate?.({ ...character, determination: character.determination - 1 });
        }

        // Persistance historique (non bloquant)
        ...
        setStep(3);
    }, [socket, activeGMSession, character, onCharacterUpdate, fetchWithAuth, apiBase]);

    const handleAnimComplete = useCallback(() => {
        setAnimSequence(null);
        if (pendingResult) {
            applyPostRoll(pendingResult);
            setPendingResult(null);
        }
    }, [pendingResult, applyPostRoll]);
                            ...
                            <>
                                ...
                                <div className="flex flex-wrap gap-2 justify-center">
                                    {rollResults.results.map((v, i) => (
                                        <DieResult key={i} value={v} rang={rollResults.rang} hasSpec={rollResults.hasSpec} competenceRang={selectedCompetence.rang} />
                                    ))}
                                </div>
